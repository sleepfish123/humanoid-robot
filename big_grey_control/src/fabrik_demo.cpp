...
int main(int argc, char **argv)
{
    ...
    while (ros::ok() && fflag == 0) {
        if (flag_server==1 && flag==1) {
            ...
            joint_group_positions[1] = -car_zuoyou; // car_x，x是左右(反的)
        ...
        if (fflag == 1) {
            ...
            robot_arm_states.position.push_back(theta_zhoubu + 90 - 2); // 右肘部+90°
            ...
            RobotArm_.arm_control(robot_arm_states); // 2222222222222222222
            
            // 再腰
            // up_down_waist = 0.05; // m
            // RobotWaist_.foot_waist_action_control(theta_ceyao, theta_wanyao, up_down_waist * 1000, (theta_jiaobu)); // 角度、mm（无补偿）
            RobotWaist_.foot_waist_action_control(0.0, 0.0, 0.0, (theta_jiaobu)); // 角度、mm（无补偿）
            
            // sleep(10); // 确保手臂运动到了再闭合爪子
            // 再爪子
            RobotClaw_.claw_close_control(); // 爪子闭合// 2222222222222222222
            sleep(2);
            // RobotClaw_.claw_close_control(); // 爪子闭合// 2222222222222222222
            // sleep(2);
            ...
            /*******************************抓完回来******************************/
            // 先手臂
            RobotArm_.arm_init(); // 手臂初始化// 2222222222222222222
            // 再腰
            // RobotWaist_.foot_waist_action_control(-theta_ceyao, -theta_wanyao, -up_down_waist * 1000, 0.0); // 角度、mm（无补偿）
            RobotWaist_.foot_waist_action_control(0.0, 0.0, 0.0, 0.0); // 角度、mm（无补偿）
            ...
            sleep(5);
            RobotClaw_.claw_open_control(); // 爪子张开// 2222222222222222222
